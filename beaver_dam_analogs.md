...
<!--Beaver dam analogs can be built using different construction techniques and materials depending on structure objectives, site characteristics, local availability of woody vegetation, and project resources. For example, depending on the flow regime, and expected peak discharges, it may not be necessary to install wooden posts. Installing untreated wooden posts in the stream bed offers additional stability, and may require less woody weave material than structures built without posts. However, installing wooden posts requires expensive equipment which is time consuming and labor-intensive to transport. Furthermore, it represents arguably the most hazardous aspect of building BDAs. Deciding whether or not to install posts therefore depends on an assessment of the expected force (i.e., stream power) the structure will be subject to, the specific goals of the structure, and the ease with which posts and post-pounder can be brought on-site.-->
